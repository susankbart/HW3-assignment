...
const colorisplayElement = document.getElementById("color_display");

const levels = Array.from(document.getElementsByClassName("mode"));

let selectedLevelButton = levels.find((level) => {
    const classsList = Array.from(level.classList);
...
// attempt to make background of all squares the same
...
startButton.addEventListener("click", function(){
    ...
    for(let i = 0; i < squares.length; i = i+1){
        ...
        square.datasetrgb_value = JSON.stringify([red, green, blue]);
     square.style.backgroundColor = rgbString;
    }
    //assigning header random valuefrom 1 of the square values
...
//add an event listene to each square so that it either disappears or changes every square's color
